    ...
    ZStack {
      BackgroundView()
        ...
        VStack (alignment: .leading) {
          ...
          // MARK: Toggle Detailed Practice State Graph on Traffic Light in HomeView
          SettingsCardView(icon: isPracticeStateDetailed ? "ellipsis.vertical.bubble" : "chart.bar.doc.horizontal",
                           text: isPracticeStateDetailed ? "Show simple practice state visualization" : "Show detailed practice state visualization")
          .onTapGesture {
            isPracticeStateDetailed.toggle()
            HapticHelper.success()
            dismiss()
          }
